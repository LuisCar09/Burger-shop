...
const DropDownMenu = ({ close, isOpen }) => {
    ...
    const item = (<div className="dropDownMenu" onClick={handleMenuItemClick} > <ul>{items.map((item, index) => { return (<li key={index}><Link to={item.link}>{item.name}</Link></li>) })}</ul></div>)
    return (
